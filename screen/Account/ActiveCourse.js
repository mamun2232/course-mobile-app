...
const ActiveCourse = ({navigation}) => {
  ...
  useEffect(() => {
    fetch(`http://192.168.31.235:5000/api/v1/courses/myCourses/${user?.email}   `)
      .then((res) => res.json())
      .then((data) => {
        if (data.success) {
  ...
  return (
    <>
    {myCourses.length !== 0 ? <ScrollView className="mt-3"  showsVerticalScrollIndicator={false}>
      <View className="flex gap-3 pl-4 pr-6">
        {myCourses?.map(({ name, url, price, quantity, category , _id}) => (
          <View
